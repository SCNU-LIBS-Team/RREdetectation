...
SVG_PATH = OUT_DIR / "温度迭代流程_优化版_v2.svg"
...
svg.append(text_block(W / 2, 142, "外层遍历多个温度起点；内层对每个起点迭代更新温度并进行全局最优比较", size=34, weight=430, fill=MUTED, stroke_width=8))
...
svg.append(text_block(inner_x + inner_w / 2, inner_y + 143, "每轮均根据当前温度 T 更新基体元素谱线库；未收敛时返回该步骤", size=30, weight=430, fill=MUTED, stroke_width=7))
svg.append(label(inner_x + inner_w - 70, inner_y + 178, "本文实验设置：max_iterations = 12，tolerance = 1e−5", size=27, anchor="end", weight=500))
...
connectors = [
    ...
    connector("M 1735 1065 L 1635 1065 L 1635 1955 L 1870 1955"),
    # 内层输出 -> 外层当前起点结果比较。
    ...
    # 更新/保持两路合流。
    connector("M 450 2255 L 450 2300 L 475 2300"),
    connector("M 1100 2255 L 1100 2300 L 1075 2300"),
    # 外层回路：完成当前起点评估 -> 外层判断。
    connector("M 775 2455 L 1360 2455 L 1360 975 L 1240 975"),
]
...
svg.append(box(*outer_end, ["输出全局最优温度与评分", "结束"], kind="terminator", size=40, weight=650))
...
svg.append(label(1475, 1165, "调用内层温度迭代", size=32, anchor="middle", weight=550))
svg.append(label(2390, 1188, "是", size=34, anchor="start"))
svg.append(label(3140, 1615, "否", size=34))
svg.append(label(3270, 1240, "未收敛，返回更新谱线库", size=30, anchor="end", weight=550))
...
svg.append(label(1285, 1480, "下一温度起点", size=32, anchor="end", weight=550))
